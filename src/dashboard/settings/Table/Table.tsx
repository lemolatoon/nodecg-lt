...
export const Table = (props: Props) => {
  const [memberInfos, setMemberInfos] = useReplicant<MemberInfo[]>(
    "members",
    [],
    { defaultValue: [] },
  );
  const [memberIndex, setMemberIndex] = useReplicant<number>("memberIndex", 0, {
    defaultValue: 0,
  });
          ...
          <ul className={tableWrapper}>
            {memberInfos.map((memberInfo, index) => (
              <li key={index} className={extendWidth}>
                <button
                  className={`${buttonReset} ${extendWidth}`}
